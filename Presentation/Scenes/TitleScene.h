...
/**
 * @brief タイトル画面のシーンクラス
 * @details IScene インターフェースを実装し、タイトル画面における入力処理、更新処理、描画処理を制御する
 */
class TitleScene : public IScene<SceneType, SceneSharedData> {
public:
    /**
     * @brief タイトルシーンの初期化処理を行う
     */
    void Initialize() override;

    /**
     * @brief タイトルシーンでのユーザー入力処理を行う
     */
    void ProcessInput() override;

    /**
     * @brief タイトルシーンの状態更新処理を行う
     */
    void Tick() override;
    /** @brief タイトルシーンのリソースを解放する */
    void Dispose() override;

    /**
     * @brief タイトルシーンの描画処理を行う
...
private:
    ...
    std::unique_ptr<Button> m_galleryButton;
    std::unique_ptr<Button> m_rankingButton;
