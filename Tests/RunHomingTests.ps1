...
$output = Join-Path $repo 'Temp/HomingBuild'
$objects = Join-Path $repo 'Temp/HomingGameObj'
& "$vs/MSBuild/Current/Bin/MSBuild.exe" "$repo/FloppyEngine.vcxproj" /p:Configuration=Release /p:Platform=x64 "/p:OutDir=$output/" "/p:IntDir=$repo/Temp/HomingEngineObj/" /m /v:minimal /nologo
...
$arguments = @('/nologo', '/std:c++20', '/EHsc', '/MT', '/utf-8', '/UNDEBUG',
    ('"' + "$PSScriptRoot/HomingShotTests.cpp" + '"'),
