...
& "$vs/MSBuild/Current/Bin/MSBuild.exe" "$repo/FloppyDiskShootingGame.vcxproj" /p:Configuration=Release /p:Platform=x64 /p:BuildProjectReferences=false "/p:DistributionDir=$repo/dist/Floppy/x64/HomingLaunchPreview/" "/p:OutDir=$output/" "/p:IntDir=$objects/" /m /v:minimal /nologo
