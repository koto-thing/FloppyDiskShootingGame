...
$output = Join-Path $repo 'Temp/Homing3DBuild'
$objects = Join-Path $repo 'Temp/Homing3DGameObj'
& "$vs/MSBuild/Current/Bin/MSBuild.exe" "$repo/FloppyEngine.vcxproj" /p:Configuration=Release /p:Platform=x64 "/p:OutDir=$output/" "/p:IntDir=$repo/Temp/Homing3DEngineObj/" /m /v:minimal /nologo
if ($LASTEXITCODE) { throw 'Engine build failed' }
& "$vs/MSBuild/Current/Bin/MSBuild.exe" "$repo/FloppyDiskShootingGame.vcxproj" /p:Configuration=Release /p:Platform=x64 /p:BuildProjectReferences=false "/p:DistributionDir=$repo/dist/Floppy/x64/Homing3DPreview/" "/p:OutDir=$output/" "/p:IntDir=$objects/" /m /v:minimal /nologo
