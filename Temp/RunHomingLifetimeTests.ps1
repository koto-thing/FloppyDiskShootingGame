...
$objects = Join-Path $repo 'Temp/HomingLifetimeGameObj'
...
& "$vs/MSBuild/Current/Bin/MSBuild.exe" "$repo/FloppyDiskShootingGame.vcxproj" /p:Configuration=Release /p:Platform=x64 /p:BuildProjectReferences=false "/p:DistributionDir=$repo/dist/Floppy/x64/HomingLifetimePreview/" "/p:OutDir=$output/" "/p:IntDir=$objects/" /m /v:minimal /nologo
